<template>
    <DataTable  paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]" :value="items">
        <Column header="Тест">
            <template #body="{ data }">
                <div class="d-flex gap-2 align-items-center">
                    <img :alt="data.title" :src="data.image" class="rounded-circle" style="width: 45px; height: 45px;" />
        ...
        <Column header="Статус">
            <template #body="{ data }">
                <Tag :value="testStatusLabel(data.test_user.status)" :severity="testStatusClass(data.test_user.status)" />
            </template>
        </Column>
        ...
        <Column>
            <template #body="slotProps">
                <Button class="p-button-sm py-1 px-2 rounded"
                        ...
                        iconPos="right" label="Открыть"
                        @click="$router.push({name: 'show-result', params: { id: slotProps.data.id }})" />
            </template>
        </Column>
    </DataTable>
</template>
