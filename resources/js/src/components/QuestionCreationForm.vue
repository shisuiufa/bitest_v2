    ...
    <div class="create-question" v-if="this.typesQuestion.options.length > 0">
        <div class="row">
            <div class="col-12">
                <div class="mb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <ui-button class="btn-remove" v-if="selectedQuestion"
                                   :class="{'ui-button_disabled' : isFormInvalid}" @click="deleteQuestion">
                            Удалить вопрос
                        </ui-button>
                    </div>
                </div>
                ...
                <div v-if="typesQuestion.selected?.value > 0" class="create-question__options options">
                    <label class="form-label">Варианты ответов</label>
                    <div v-for="(option, index) in options" :key="index" class="options__item">
                        <input v-if="typesQuestion.selected?.value === 1" :id="'option-radio-' + (index + 1)"
                               :value="index"
                               @change="this.updateCorrectOption(option)"
                               :checked="option.correct"
                               name="option-radio"
                               class="form-check-input" type="radio">
                        <input v-if="typesQuestion.selected?.value === 2" :id="'option-checkbox-' + (index + 1)"
                               v-model="option.correct" :name="'option-' + index"
                               class="form-check-input" type="checkbox">
                        <custom-input v-model:modelValue="option.name" type="text"></custom-input>
                        <ui-button v-if="index > 1" @click="deleteOption(index)" class="options__btn-delete"><i
                            class="bi bi-x-lg"></i></ui-button>
                    </div>
                </div>
            </div>
            <div class="col-12 d-flex justify-content-between">
                ...
                <ui-button v-if="selectedQuestion" :class="{'ui-button_disabled' : isFormInvalid}"
                           @click="editQuestion">
                    Сохранить
                </ui-button>
